...
// 方式 1: 使用 HTTP API（通過 Aggregator/Publisher）- 簡單但功能有限
// 測試網配置
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const WALRUS_CONFIG_HTTP = {
  // 使用 Walrus 測試網的公共服務
  aggregatorUrl: 'https://aggregator.walrus-testnet.walrus.space',
  publisherUrl: 'https://publisher.walrus-testnet.walrus.space', // 可選
}

// 方式 2: 使用 Walrus SDK（推薦）- 功能完整，需要 SuiClient 和 Signer
// 注意：如需使用 SDK 模式，取消註釋下面的配置並切換 WALRUS_CONFIG
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const WALRUS_CONFIG_SDK = {
  // 不提供 aggregatorUrl 將自動使用 SDK 模式
  network: 'testnet' as const, // 測試網配置
  // 測試網 Sui RPC URL
  suiRpcUrl: 'https://fullnode.testnet.sui.io:443',
  // 可選：使用 Upload Relay 來減少客戶端請求（測試網）
  uploadRelay: {
    host: 'https://upload-relay.testnet.walrus.space',
    sendTip: {
      max: 1000, // 最大小費（MIST）- 測試網使用較小的小費
    },
  },
  // 可選：存儲節點客戶端選項（參考範例代碼）
...
// 選擇使用哪種配置
// 使用 HTTP API 方式（推薦，簡單快速）
// const WALRUS_CONFIG = WALRUS_CONFIG_HTTP
// 或使用 SDK 方式（需要實現 Signer 適配器，見下方說明）
const WALRUS_CONFIG = WALRUS_CONFIG_SDK

const SEAL_CONFIG = {
  // Seal Key Server URL - 請參考 https://seal-docs.wal.app/ 獲取實際 URL
  keyServerUrl: 'https://seal-key-server.example.com', // 需要替換為實際的 Seal key server URL
  accessPolicyPackageId: undefined, // 可選：Sui Move package ID
}
...
function App() {
  ...
  const [filePreview, setFilePreview] = useState<string | null>(null) // 圖片預覽 URL
  const [uploadMode, setUploadMode] = useState<'text' | 'file'>('text') // 上傳模式
  const [encryptedBlobRef, setEncryptedBlobRef] = useState<string | null>(null)
  const [blobIdInput, setBlobIdInput] = useState<string>('') // 用戶輸入的 blobId
  ...
  const [currentPage, setCurrentPage] = useState<'walrus' | 'wallet-test' | 'seal-test'>('walrus')

  // 使用 useMemo 確保服務實例不會在每次渲染時重新創建
  ...
  useEffect(() => {
    const checkBalance = async () => {
      ...
      try {
        ...
        setSuiBalance((Number(totalSui) / 1e9).toFixed(4)) // 轉換為 SUI

        // 檢查 WAL 餘額（測試網 WAL token coin type）
        // 根據錯誤信息，測試網使用的 WAL coin type：
        ...
        try {
          ...
          setWalBalance(walBalanceNum.toFixed(4)) // 轉換為 WAL
          
          // 檢查 coin 碎片化問題
          if (walCoins.data.length > 1) {
            console.warn(`⚠️ 檢測到 ${walCoins.data.length} 個 WAL coin，可能存在碎片化問題`)
            console.log('WAL coins:', walCoins.data.map(c => ({
              balance: (Number(c.balance) / 1e9).toFixed(4),
              coinObjectId: c.coinObjectId
            })))
          }
        } catch (err) {
          // 如果沒有 WAL token，設置為 0
          setWalBalance('0')
  ...
  const detectFileType = (data: Uint8Array): { type: string; extension: string } => {
    ...
    // SVG: 檢查是否包含 <svg 標籤（前幾個字節可能是 UTF-8 BOM 或空白）
   ...
   * 加密並上傳數據到 Walrus
   * 支持兩種方式：
   * 1. HTTP API 方式（通過 Aggregator/Publisher）
   * 2. SDK 方式（直接使用 Walrus SDK，需要 Signer）
   */
  ...
  const handleDownloadAndDecrypt = async () => {
    // 使用輸入框中的 blobId，如果為空則使用上次上傳的 blobId
  ...
  // 頁面切換
  if (currentPage === 'wallet-test') {
    return (
      <div className="app">
        <div className="container">
          <div style={{ marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <button
              onClick={() => setCurrentPage('walrus')}
            ...
            <button
              onClick={() => setCurrentPage('seal-test')}
              className="btn btn-secondary"
